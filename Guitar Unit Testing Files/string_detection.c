...
void string_detection_init(void) {
    ...
    printf("Number of notes in database: %d\n", NUM_NOTES);
}
...
/**
 * Find the closest guitar string based on frequency
 * Uses predefined string frequency ranges
 * 
 * @param frequency: Detected frequency
 * @param closest_freq: Output parameter for ideal string frequency
 * @return: String number (1-6) or -1 if no match
 */
int find_closest_string(double frequency, double* closest_freq) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (diff < min_diff) {
            min_diff = diff;
            closest_string = i + 1; // Convert to 1-based indexing
